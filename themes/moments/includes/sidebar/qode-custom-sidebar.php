...
if ( ! class_exists( 'MomentsQodefSidebar' ) ) {

	class MomentsQodefSidebar {
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'moments' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'moments-qodef-sidebar', QODE_ROOT . '/framework/admin/assets/js/qodef-sidebar.js' );
			wp_enqueue_style( 'moments-qodef-sidebar', QODE_ROOT . '/framework/admin/assets/css/qodef-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__( 'Enter Name of the new Widget Area', 'moments' ) . "' name='qode-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__( 'Add Widget Area', 'moments' ) . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function add_sidebar_area() {
			if ( ! empty( $_POST['qode-add-widget'] ) ) {
				...
				$name           = rtrim( $this->get_name( sanitize_text_field( $_POST['qode-add-widget'] ) ) );
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'moments_qodef_custom_widget_args', $args );
