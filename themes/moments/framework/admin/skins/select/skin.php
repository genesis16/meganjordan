...
class SelectSkin extends MomentsQodefSkinAbstract {
    /**
     * Skin constructor. Hooks to moments_qodef_admin_scripts_init and moments_qodef_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'select';

        //hook to admin register scripts
        add_action('moments_qodef_admin_scripts_init', array($this, 'registerStyles'));
        add_action('moments_qodef_admin_scripts_init', array($this, 'registerScripts'));

        //hook to admin enqueue scripts
        add_action('moments_qodef_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('moments_qodef_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('moments_qodef_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('moments_qodef_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));

		add_action( 'admin_enqueue_scripts', array( $this, 'setShortcodeJSParams' ), 5 ); // 5 is set to 
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            moments_qodef_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            moments_qodef_register_skin_style($styleHandle, $stylePath);
        }
	...
	public function setIcons() {
        ...
        $icons_array = apply_filters('moments_qodef_set_skin_icons', $icons_array, $this->getSkinURI() . '/');
	...
	public function setMenuItemPosition() {
        ...
        $positions_array = apply_filters('moments_qodef_set_skin_menu_positions', $positions_array);
	...
	public function renderOptions() {
        ...
        $tab    = moments_qodef_get_admin_tab();
        $active_page = $moments_qodef_Framework->qodeOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param MomentsQodefAdminPage $page current page to load
     *
     * @see QodeSkinAbstract::loadTemplatePart()
     */
