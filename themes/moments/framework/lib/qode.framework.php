...
class MomentsQodefFramework {
	...
	private function __construct() {
		$this->qodeOptions   = MomentsQodefOptions::get_instance();
		$this->qodeMetaBoxes = MomentsQodefMetaBoxes::get_instance();
	}
	...
	public function setSkin( MomentsQodefSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class MomentsQodefSkinManager
 *
...
/**
 * Class MomentsQodefSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class MomentsQodefSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( moments_qodef_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
        <script>
            window.qodeSCIcon = '<?php echo moments_qodef_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
class MomentsQodefOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $moments_qodef_options ) ) {
			return $moments_qodef_options[ $key ];
...
/*
   Class: MomentsQodefAdminPage
   A class that initializes Qode Admin Page
*/

class MomentsQodefAdminPage implements iMomentsQodefLayoutNode {
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new MomentsQodefFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iMomentsQodefRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: MomentsQodefMetaBoxes
...
/*
   Class: MomentsQodefMetaBox
   A class that initializes Qode Meta Box
*/

class MomentsQodefMetaBox implements iMomentsQodefLayoutNode {
	...
	public function setTitle( $label ) {
		...
		return $moments_qodef_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

if ( ! function_exists( 'moments_qodef_init_framework_variable' ) ) {

	function moments_qodef_init_framework_variable() {

		global $moments_qodef_Framework;

		$moments_qodef_Framework = MomentsQodefFramework::get_instance();
		$qodeSkinManager         = new MomentsQodefSkinManager();
		$moments_qodef_Framework->setSkin( $qodeSkinManager->getSkin() );
	}

	add_action( 'moments_qodef_before_options_map', 'moments_qodef_init_framework_variable' );
}
?>