...
if ( ! function_exists( 'moments_qodef_single_portfolio' ) ) {
	function moments_qodef_single_portfolio() {
		$portfolio_template = moments_qodef_get_portfolio_single_type();
		...
		if ( $portfolio_template == 'gallery' ) {
			$params['holder_class'][] = 'qodef-portfolio-gallery-' . moments_qodef_options()->getOptionValue( 'portfolio_single_numb_columns' );
		}

		moments_qodef_get_module_template_part( 'templates/single/holder', 'portfolio', '', $params );
	}
}

if ( ! function_exists( 'moments_qodef_get_portfolio_single_type' ) ) {
	function moments_qodef_get_portfolio_single_type() {
		return moments_qodef_get_meta_field_intersect( 'portfolio_single_template' );
	}
}

if ( ! function_exists( 'moments_qodef_get_portfolio_single_media' ) ) {
	function moments_qodef_get_portfolio_single_media() {
		...
		if ( is_array( $videos ) && count( $videos ) ) {
			usort( $videos, 'moments_qodef_compare_portfolio_videos' );
			foreach ( $videos as $video ) {
				...
				if ( ! empty( $video['portfoliovideotype'] ) ) {
					...
					$media['video_url']   = moments_qodef_portfolio_get_video_url( $video );
...
if ( ! function_exists( 'moments_qodef_portfolio_get_video_url' ) ) {
	function moments_qodef_portfolio_get_video_url( $video ) {
...
if ( ! function_exists( 'moments_qodef_portfolio_get_media_html' ) ) {
	function moments_qodef_portfolio_get_media_html( $media ) {
		global $wp_filesystem;
		$params = array();

		//For adding image overlay in gallery template type
		$params['gallery'] = ( moments_qodef_get_portfolio_single_type() == 'gallery' ) ? true : false;

		if ( $media['type'] == 'image' ) {
			$params['lightbox'] = moments_qodef_options()->getOptionValue( 'portfolio_single_lightbox_images' ) == 'yes';
		...
		if ( in_array( $media['type'], array( 'youtube', 'vimeo' ) ) ) {
			$params['lightbox'] = moments_qodef_options()->getOptionValue( 'portfolio_single_lightbox_videos' ) == 'yes';
		...
		moments_qodef_get_module_template_part( 'templates/single/media/' . $media['type'], 'portfolio', '', $params );
	}
}

if ( ! function_exists( 'moments_qodef_compare_portfolio_videos' ) ) {
	/**
	 ...
	 */
	function moments_qodef_compare_portfolio_videos( $a, $b ) {
...
if ( ! function_exists( 'moments_qodef_compare_portfolio_options' ) ) {
	/**
	 ...
	 * @return int result of comparison
	 */
	function moments_qodef_compare_portfolio_options( $a, $b ) {
...
if ( ! function_exists( 'moments_qodef_portfolio_get_info_part' ) ) {
	function moments_qodef_portfolio_get_info_part( $part ) {
		$portfolio_template = moments_qodef_get_portfolio_single_type();

		moments_qodef_get_module_template_part( 'templates/single/parts/' . $part, 'portfolio', $portfolio_template );
	}
}