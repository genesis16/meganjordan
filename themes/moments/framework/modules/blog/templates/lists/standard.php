<div class="<?php echo esc_attr($blog_classes)?>"   <?php echo esc_attr($blog_data_params) ?> >
	...
	if($blog_query->have_posts()) : while ( $blog_query->have_posts() ) : $blog_query->the_post();
		moments_qodef_get_post_format_html($blog_type);
	endwhile;

	else:
		moments_qodef_get_module_template_part('templates/parts/no-posts', 'blog');
	endif;

	if(moments_qodef_options()->getOptionValue('pagination') == 'yes') {
        if (moments_qodef_options()->getOptionValue('pagination_type') == 'navigation') {
            ?>
            ...
            moments_qodef_pagination($blog_query->max_num_pages, $blog_page_range, $paged);
        }
	}
	?>
</div>
