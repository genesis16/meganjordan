<?php
namespace MomentsQodef\Modules\Shortcodes\Icon;

use MomentsQodef\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package MomentsQodef\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function render($atts, $content = null) {
        ...
        $html = select_core_get_shortcode_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.moments_qodef_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.moments_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.moments_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.moments_qodef_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.moments_qodef_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.moments_qodef_filter_px($params['border_radius']).'px';
                }
            }
        }

        else if(isset($params['type']) && $params['type'] === 'normal') {
            if(isset($params['margin']) && $params['margin'] !== '') {
                $iconHolderStyles[] = 'margin: '.$params['margin'];
            }
        }
