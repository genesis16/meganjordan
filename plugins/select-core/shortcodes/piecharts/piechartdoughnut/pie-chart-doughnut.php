<?php
namespace MomentsQodef\Modules\Shortcodes\PieCharts\PieChartDoughnut;
...
class PieChartDoughnut implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ($i = 1; $i <= $this->chartFields; $i++) {
			...
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__( 'Chart Value ', 'select-core' ) . $i,
				'param_name' => "chart_value_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'colorpicker',
				'heading' => esc_html__( 'Chart Color ', 'select-core' ) . $i,
				'param_name' => "chart_color_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__( 'Chart Legend ', 'select-core' ) . $i,
		...
		vc_map( array(
			'name' => esc_html__('Select Pie Chart 3 (Doughnut)', 'select-core'),
			'base' => $this->getBase(),
			...
			'params' => array_merge(
				array(
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Height', 'select-core' ),
						'param_name' => 'height',
					),
				),
				$chart_fields
	...
	public function render($atts, $content = null) {
		...
		$html = select_core_get_shortcode_template_part('templates/pie-chart-doughnut', 'piecharts/piechartdoughnut', '', $params);
