...
class MomentsCoreImport {
	...
	public function set_import_url() {
		$params = MomentsCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
				...
				if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
					...
					switch ($_POST['options']['action']):
						...
						case 'menu-settings':
							...
							break;
						case 'rev-slider':
							if (moments_core_is_revolution_slider_installed()) {
								$this->rev_slider_import($demo);
							} else {
								$this->set_status('success');
								$this->set_data('type', 'options');
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'moments-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!MomentsCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									moments_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'moments-core'));
									exit;
								}
							}
			...
			moments_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'moments-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $moments_widget_id => $moments_widget_data) {
				update_option('widget_' . $moments_widget_id, $moments_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'moments-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$moments_sidebars = get_option( "sidebars_widgets" );
		unset( $moments_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$moments_sidebars = array_merge( (array) $moments_sidebars, (array) $data['sidebars'] );
			unset( $moments_sidebars['wp_inactive_widgets'] );
			$moments_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $moments_sidebars );
			$moments_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $moments_sidebars );
			return true;
		} else {
			return false;
		}
		...
		if($options){
			if($current_options != $options) {
				...
				if ($result) {
					...
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'moments-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'moments-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'moments-core'));
			}
		}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_moments_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'moments-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'moments-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'moments-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'moments-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'moments-core' ) );
		}
	}
...
//		}
//
		if(moments_core_is_woocommerce_installed()) {
		...
		$moments_import = new WP_Import();
		set_time_limit( 0 );

		$moments_import->fetch_attachments = $attachments;
		$returned_value                  = $moments_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'moments-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'moments-core' ) . ' ' . $xml );
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'moments-core'));
					exit;
				}
			}
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'moments-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'moments-core'));
		}
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = moments_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					moments_core_ajax_status( 'success', '', $html);
				} else {
					moments_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'moments-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = MomentsCoreSystemInfoPage::get_instance()->get_system_info();
...
MomentsCoreImport::get_instance();