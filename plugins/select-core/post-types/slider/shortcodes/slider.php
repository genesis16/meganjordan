...
namespace QodeCore\PostTypes\Slider\Shortcodes;
...
class Slider implements Lib\ShortcodeInterface {
	...
	public function render( $atts, $content = null ) {
		...
		if ( $slider != "" ) {
			...
			if ( isset( $slider_meta['anchor'] ) && $slider_meta['anchor'] != '' ) {
				$slider_anchor = 'data-qodef-anchor="' . $slider_meta['anchor'] . '"';
			}
			...
			switch ( $animation_type ) {
				...
				case 'slide-cover':
					...
					break;
				case 'slide-peek':
					$animation_type_class = 'qodef-slide-peek';
					break;
				default:
					$animation_type_class = 'qodef-slide';
			}
			/* check for slide animation type - end */

			/* set slider preloader - start */
			if ( $moments_qodef_options['smooth_page_transitions'] == "yes" && $moments_qodef_options['smooth_pt_spinner_type'] != "" ) {
				$ajax_loader = '<div class="qodef-st-loader"><div class="qodef-st-loader1">' . moments_qodef_loading_spinners( true ) . '</div></div>';
			} else {
				$ajax_loader = '<div class="qodef-st-loader"><div class="qodef-st-loader1">' . moments_qodef_loading_spinner_pulse() . '</div></div>';
			}

			/* set slider preloader - end */

			/* set padding for slider arrows - start */
			$slider_arrows_padding = "padding-top: " . esc_attr( $this->moments_qodef_get_slider_navigation_padding() ) . "px;";
			/* set padding for slider arrows - end */

			$html .= '<div class="qodef-slider">';
			$html .= '<div class="qodef-slider-inner">';
			$html .= '<div id="qodef-' . esc_attr( $slider ) . '" ' . $slider_anchor . ' class="carousel slide ' . esc_attr( $animation_type_class . ' ' . $full_screen_class . ' ' . $responsive_height_class . ' ' . $height_class . ' ' . $auto_start_class . ' ' . $header_effect_class . ' ' . $slider_numbers_class . ' ' . $slider_thumbs_class ) . ' " ' . $slide_animation_timeout . ' ' . $data_height . ' ' . $data_parallax_effect . ' ' . moments_qodef_get_inline_style( $slide_holder_height ) . '><div class="qodef-slider-preloader">' . $ajax_loader . '</div>';
			$html .= '<div class="carousel-inner ' . esc_attr( $slider_css_position_class ) . '" ' . moments_qodef_get_inline_style( $carouselinner_height ) . ' ' . $data_parallax_transform . '>';
			...
			if ( have_posts() ) : $postCount = 0;
				while ( have_posts() ) : the_post();
					...
					if ( get_post_meta( get_the_ID(), "qodef_enable_image_animation", true ) == "yes" ) {
						...
						$animate_image_class .= ( $meta_temp = get_post_meta( get_the_ID(), "qodef_enable_image_animation_type", true ) );
						$animate_image_data  .= "data-qodef_animate_image='" . $meta_temp . "'";
					}
					...
					$header_style = "";
					if ( ( $meta_temp = get_post_meta( get_the_ID(), "qodef_slide_header_style", true ) ) != "" ) {
					...
					if ( $slide_type == 'video' ) {
						...
						$html .= '<div class="qodef-video"><div class="qodef-mobile-video-image" ' . moments_qodef_get_inline_style( 'background-image: url(' . esc_url( $video_image ) . ')' ) . '></div><div class="qodef-video-overlay';
						...
						$html .= '<video class="qodef-video-element" width="1920" height="800" poster="' . esc_url( $video_image ) . '" controls="controls" preload="auto" loop autoplay muted>';
						if ( ! empty( $video_webm ) ) {
						...
						$html .= '<object width="320" height="240" type="application/x-shockwave-flash" data="' . esc_url( get_template_directory_uri() ) . '/assets/js/flashmediaelement.swf">
                                                    ...
                                                    <img src="' . esc_url( $video_image ) . '" width="1920" height="800" title="' . esc_attr__( 'No video playback capabilities', 'select-core' ) . '" alt="' . esc_attr__( 'Video thumb', 'select-core' ) . '" />
                                            </object>
                                    </video>
                            </div></div>';
					} else {
						$html .= '<div class="qodef-image" ' . moments_qodef_get_inline_style( 'background-image:url(' . esc_url( $image ) . ')' ) . '>';
						if ( $slider_thumbs == 'no' ) {
							$html .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr( $title ) . '">';
						}

						if ( $image_overlay_pattern !== "" ) {
							$html .= '<div class="qodef-image-pattern" ' . moments_qodef_get_inline_style( 'background: url(' . esc_url( $image_overlay_pattern ) . ') repeat 0 0' ) . '></div>';
						}
						$html .= '</div>';
					}
					...
					$element_positions = get_post_meta( get_the_ID(), 'qodef_slide_holder_elements_alignment', true );
					if ( ! strlen( $element_positions ) ) {
						$element_positions = 'center';
					}

					$frame_in_grid = get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_in_grid', true );
					if ( ! strlen( $frame_in_grid ) ) {
						$frame_in_grid = 'no';
					}

					$def_width = get_post_meta( get_the_ID(), 'qodef_slide_elements_default_width', true );
					if ( ! strlen( $def_width ) ) {
						$def_width = '1920';
					}
					...
					if ( $element_positions == 'custom' ) {
						$html .=
							'<div class="qodef-slider-elements-container qodef-custom-elements ' . ( $frame_in_grid == 'yes' ? 'qodef-grid' : '' ) . '" ' . moments_qodef_get_inline_style( ( $frame_in_grid == 'no' && ( ( $meta_temp = get_post_meta( get_the_ID(), "qodef_slide_holder_frame_width", true ) ) != '' ) ? 'width: ' . $meta_temp . '%; ' : '' ) . 'padding-top: ' . esc_attr( $this->moments_qodef_get_slider_navigation_padding() ) . 'px;' ) . ' ' . $slide_elements_data_start . ' ' . $slide_elements_data_end . ( $frame_in_grid == 'yes' ? '' : ' data-width-mobile="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_mobile', true ) ) . '" data-width-tablet-p="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_tablet_p', true ) ) . '" data-width-tablet-l="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_tablet_l', true ) ) . '" data-width-laptop="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_laptop', true ) ) . '" data-width-desktop="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_desktop', true ) ) . '"' ) . '>' .
							'<div class="qodef-slider-elements-container-inner" ' . moments_qodef_get_inline_style( ( ( $meta_temp = get_post_meta( get_the_ID(), "qodef_slide_holder_frame_height", true ) ) != '' ) ? 'padding-bottom: ' . $meta_temp . '%;' : '' ) . '">' .
							'<div class="qodef-slider-elements-holder-frame" ' . moments_qodef_get_inline_style( 'width: 100%; height: 100%' ) . ' data-default-width="' . esc_attr( $def_width ) . '">';
					} else {
						$html .=
							'<div class="qodef-slider-elements-container" ' . moments_qodef_get_inline_style( 'width: 100%; box-sizing: border-box; height: 100%; ' . 'padding-top: ' . esc_attr( $this->moments_qodef_get_slider_navigation_padding() ) . 'px;' ) . ' ' . $slide_elements_data_start . ' ' . $slide_elements_data_end . ( $frame_in_grid == 'yes' ? '' : ' data-width-mobile="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_mobile', true ) ) . '" data-width-tablet-p="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_tablet_p', true ) ) . '" data-width-tablet-l="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_tablet_l', true ) ) . '" data-width-laptop="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_laptop', true ) ) . '" data-width-desktop="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_desktop', true ) ) . '"' ) . '>' .
							'<div class="qodef-slider-elements-container-inner" ' . moments_qodef_get_inline_style( 'height: 100%;' ) . '>' .
							'<div class="qodef-slider-elements-holder-frame ' . ( $frame_in_grid == 'yes' ? 'qodef-grid' : '' ) . '" ' . moments_qodef_get_inline_style( ( $frame_in_grid == 'no' && ( ( $meta_temp = get_post_meta( get_the_ID(), "qodef_slide_holder_frame_width", true ) ) != '' ) ? 'width: ' . $meta_temp . '%; ' : '' ) . 'top: 50%; left: 50%; -webkit-transform: translate(-50%,-50%); transform: translate(-50%,-50%);' ) . ' data-default-width="' . esc_attr( $def_width ) . '">';
					}

					$default_animation = get_post_meta( get_the_ID(), 'qodef_slide_elements_default_animation', true );
					if ( ! strlen( $default_animation ) ) {
						$default_animation = 'none';
					}

					// These keys and values have to match those in qode.layout.php, map.php (for slider) and shortcodes.js
					...
					$slide_bottom_html = '';
					$no                = 1;
					$slide_elements    = get_post_meta( get_the_ID(), 'qode_slide_elements', true );
					while ( isset( $slide_elements[ $no - 1 ] ) ) {
						$slide_element = $slide_elements[ $no - 1 ];

						if ( $slide_element['slideelementvisible'] == 'yes' ) {
							$slide_element_html = '';
							$class_string       = 'qodef-el qodef-slide-element';
							$data_string        = '';
							$style_string       = '';
							$inner_style_string = '';
							...
							if ( $slide_element['slideelementanimation'] == 'default' ) {
								if ( $default_animation != 'none' ) {
									$class_string .= ' qodef-slide-element-animation-' . $default_animation;
								...
								$class_string .= ' qodef-slide-element-animation-' . $slide_element['slideelementanimation'];
								if ( $slide_element['slideelementanimationdelay'] != '' ) {
									$inner_style_string .= '-webkit-animation-delay: ' . esc_attr( $slide_element['slideelementanimationdelay'] ) . '; animation-delay: ' . esc_attr( $slide_element['slideelementanimationdelay'] ) . ';';
								}
								if ( $slide_element['slideelementanimationduration'] != '' ) {
							...
							switch ( $slide_element['slideelementtype'] ) {
								case 'text':
									$class_string   .= ' qodef-slide-element-text';
									...
									$class_string .= ' ' . 'qodef-slide-element-text-' . $slide_element['slideelementtextstyle'];
									...
									if ( $text_options == 'advanced' ) {
										...
										if ( $slide_element['slideelementtextbgndcolor'] != '' ) {
											...
											$style_string .= 'background-color: ' . moments_qodef_rgba_color( $slide_element_bg_color, $slide_element_bg_transparency ) . ';';
										}
									...
									$class_string .= ' ' . 'qodef-slide-element-responsive-text';
									...
									$slide_element_html .=
										'<div class="' . $class_string . '" ' . $data_string . ' style="' . $style_string . '"><div class="qodef-slide-element-inner" style="' . $inner_style_string . '">' .
										( $slide_element['slideelementtextlink'] != '' ? '<a class="qodef-slide-element-wrapper-link inheriting" style="' . ( $slide_element['slideelementtextlinkhovercolor'] != '' ? 'color: ' . esc_attr( $slide_element['slideelementtextlinkhovercolor'] ) : '' ) . '" href="' . esc_url( $slide_element['slideelementtextlink'] ) . '" target="' . esc_attr( $slide_element['slideelementtexttarget'] ) . '">' : '' ) .
								...
								case 'image':
									$class_string .= ' qodef-slide-element-image';
									...
									$class_string .= ' ' . 'qodef-slide-element-responsive-image';
									$data_string  .= $responsiveness_data;
									...
									$slide_element_html .=
										'<div class="' . $class_string . ' ' . ( $element_positions . '-image' ) . '" ' . $data_string . ' style="' . $style_string . '"><div class="qodef-slide-element-inner" style="' . $inner_style_string . '">' .
										( $slide_element['slideelementimagelink'] != '' ? '<a class="qodef-slide-element-wrapper-link inheriting" href="' . esc_url( $slide_element['slideelementimagelink'] ) . '" target="' . esc_attr( $slide_element['slideelementimagetarget'] ) . '">' : '' ) .
										'<img ' . ( ( $element_positions != 'custom' && $slide_element['slideelementimagewidth'] != '' ) ? 'style="width: ' . esc_attr( $slide_element['slideelementimagewidth'] ) . '%" ' : '' ) . 'src="' . ( isset( $slide_element['slideelementimageurl'] ) ? $slide_element['slideelementimageurl'] : '' ) . '" alt="' . esc_attr__( 'Slide graphic', 'select-core' ) . '">' .
										( $slide_element['slideelementimagelink'] != '' ? '</a>' : '' ) .
										'</div></div>';
									break;

								case 'button':
									$class_string  .= ' qodef-slide-element-button';
									$button_params = array();
									...
									if ( $slide_element['slideelementbuttoniconpack'] != '' ) {
										...
										if ( $button_params['icon_pack'] != 'no_icon' ) {
											$iconPackName = moments_qodef_icon_collections()->getIconCollectionParamNameByKey( $button_params['icon_pack'] );
									...
									if ( $element_positions != 'custom' && $slide_element['slideelementbuttoninline'] == 'yes' ) {
										$class_string .= ' ' . 'qodef-slide-element-button-inline';
									}
									...
									$class_string .= ' ' . 'qodef-slide-element-responsive-button';
									$data_string  .= $responsiveness_data;

									$slide_element_html .= '<div class="' . $class_string . '" ' . $data_string . ' style="' . $style_string . '"><div class="qodef-slide-element-inner" style="' . $inner_style_string . '">' . moments_qodef_get_button_html( $button_params ) . '</div></div>';
									break;

								case 'section-link':
									$class_string         .= ' qodef-slide-element-section-link';
									...
									$responsiveness_class .= 'qodef-slide-element-responsive-text';

									$slide_bottom_html .=
										'<div class="' . $class_string . '" ' . $data_string . ' style="' . $style_string . '"><div class="qodef-slide-element-inner" style="' . $inner_style_string . '">' .
										'<div class="qodef-slide-anchor-holder">' .
										'<a class="qodef-slide-anchor-button qodef-anchor ' . $responsiveness_class . '" ' . $responsiveness_data . 'href="' . esc_url( $slide_element['slideelementsectionlinkanchor'] ) . '">' .
					...
					$html .=
						'</div>' . // closing .qodef-slider-elements-holder-frame
						'</div>' . // closing .qodef-slider-elements-container-inner
						'</div>' // closing .qodef-slider-elements-container
					;
				...
				$html .= esc_html__( 'Sorry, no slides matched your criteria.', 'select-core' );
			endif;
			wp_reset_query();
			...
			if ( $found_slides > 1 ) {
				...
				if ( $show_navigation_circles == "yes" ) {
					...
					if ( have_posts() ) : $postCount = 0;
						while ( have_posts() ) : the_post();
							...
							$slide_type = get_post_meta( get_the_ID(), "qodef_slide_background_type", true );
							if ( $slide_type == 'image' ) {
								$img .= moments_qodef_generate_thumbnail( '', esc_url( get_post_meta( get_the_ID(), "qodef_slide_image", true ) ), '160', '100' );
							} elseif ( $slide_type == 'video' ) {
								$img .= moments_qodef_generate_thumbnail( '', esc_url( get_post_meta( get_the_ID(), "qodef_slide_video_image", true ) ), '160', '100' );
							}
							$html .= '>';
							$html .= $img;
							$html .= '</li>';
				...
				if ( $show_navigation_arrows == "yes" ) {
					...
					$html .= '<a class="left carousel-control" style="' . esc_attr( $slider_arrows_padding ) . '" href="#qodef-' . esc_attr( $slider ) . '" data-slide="prev" data-start="opacity: 1;" data-300="opacity:0;">';
					if ( $slider_thumbs == 'yes' ) {
						$html .= '<span class="qodef-thumb-holder"><span class="qodef-thumb-arrow arrow_carrot-left"></span><span class="qodef-numbers"><span class="prev"></span><span class="max-number"> / ' . esc_html( $postCount ) . '</span></span><span class="img"></span></span>';
					}
					...
					$html .= '<a class="right carousel-control" style="' . esc_attr( $slider_arrows_padding ) . '" href="#qodef-' . esc_attr( $slider ) . '" data-slide="next" data-start="opacity: 1;" data-300="opacity:0;">';
					if ( $slider_thumbs == 'yes' ) {
						$html .= '<span class="qodef-thumb-holder"><span class="qodef-numbers"> <span class="next"></span><span class="max-number"> / ' . esc_html( $postCount ) . '</span></span><span class="qodef-thumb-arrow arrow_carrot-right"></span><span class="img"></span></span>';
					}
			...
			$html .= '</div>'; //close of div.qodef-slider-inner
			$html .= '</div>'; //close of div.qodef-slider
		}
	 ...
	 **/
	function moments_qodef_get_slider_item_padding() {
		return apply_filters( 'moments_qodef_slider_item_padding', 0 );
	}

	/**
	 * Function that returns slider navigation pading
	 **/
	function moments_qodef_get_slider_navigation_padding() {
		return apply_filters( 'moments_qodef_slider_navigation_padding', 0 );
	}
}